...
conn <- dbConnect(SQLite(), "/Users/asdishalla/Desktop/capstone-the-north-Gagnagrunnur 2/data/rotten_tomatoes.db")
...
actors_info <- dbGetQuery(conn, "
  SELECT headshot, actor_name, mini_biography, trivia_1, trivia_2, highest_paid_movie, highest_salary
  FROM actors_info
")
...
tabPanel(
  "Einkunnadreifing fyrir leikstjóra",
  fluidRow(
    column(
      3,
      tags$div(
        ...
        tags$p("Upplýsingar", style = "font-size: 16px;"),
        tags$p(
          "Þetta graf sýnir dreifingu einkunna fyrir leikstjóra topp 10 mynda Kate Winslet og Leonardo DiCaprio, byggt á gagnrýnendaeinkunn (Tomatometer) eða áhorfendaeinkunn.",
          style = "font-size: 14px; line-height: 1.5;"
        ),
        tags$p(
        ...
        selectInput(
          ...
          "Veldu einkunn:",
          choices = c(
            "Tomatometer einkunn" = "tomatometer_rating",
            "Áhorfenda einkunn" = "audience_rating"
          ),
...
output$rating_runtime_plot <- renderPlotly({
  ...
  ggplotly(p, tooltip = "text") %>%
    layout(hoverlabel = list(align = "left"))  # Stillir tooltip á texta
})
...
output$audience_rating_plot <- renderPlotly({
  ...
  p <- ggplot(plot_data, aes(x = audience_rating, y = audience_count)) +
    ...
    scale_y_log10() +  # Logariþmískur kvarði á y-ás
    scale_color_manual(
      values = c(
...
output$directorPlot <- renderPlotly({
  ...
  p <- ggplot(data, aes_string(x = "directors", y = rating_type)) +
    geom_boxplot(
      ...
      fill = ifelse(rating_type == "tomatometer_rating", "magenta", "seagreen1"), 
      outlier.shape = NA
    ) +
    geom_point(
