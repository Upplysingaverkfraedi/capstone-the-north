...
graph_data <- reactive({
  ...
  data <- if (rating_type == "tomatometer_rating") {
    all_director_movies %>%
      mutate(
  ...
  # raða leikstjórum eftir meðal einkunum
  data <- data %>%
    group_by(directors) %>%
    mutate(avg_rating = mean(ifelse(rating_type == "tomatometer_rating", tomatometer_rating, audience_rating), na.rm = TRUE)) %>%
    ungroup()
  
  data$directors <- factor(data$directors, levels = data %>%
                             group_by(directors) %>%
